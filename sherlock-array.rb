...
def sum_left_equal_sum_right?
  ...
  numbers_array.each_with_index do |num, index|
    ...
    if index == 0
      next
      p "nothing to the left"
    elsif index == numbers_array.length - 1
      break
    elsif index == 1
      left_range = numbers_array[0]
      left_sum = left_range
      p "left range is #{left_range} for a sum of #{left_sum}"
      right_range = (numbers_array[index + 1]..numbers_array[-1])
      right_sum = right_range.reduce(:+)
      p "right range is #{right_range.to_a} for a sum of #{right_sum}"
    else
      left_range = (numbers_array[0]...numbers_array[index])
      right_range = (numbers_array[index + 1]..numbers_array[-1])
      left_sum = left_range.reduce(:+)
      right_sum = right_range.reduce(:+)
      p "left_range is #{left_range.to_a} for a sum of #{left_sum}"
      p "right_range is #{right_range.to_a} for a sum of #{right_sum}"
    end
  end
  p "post-loop, right sum is #{right_sum} and left sum is #{left_sum}"
  if left_sum == right_sum && left_sum > 0 && right_sum > 0
    puts "YES"
  else
    puts "NO"
  end
